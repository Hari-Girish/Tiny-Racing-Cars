...
from patel_distance_race_new import runSpeedRace
...
def distanceRaceStats(raceHistory): # Function to get the total race stats
    ...
    lines.append("=" * 70)
    lines.append("Distance Race Statistics")
    lines.append("=" * 70)
    ...
    with open("patel_distance_race_new_stats_results.txt", "w") as f:
...
def carStats(carHistory, carName, fps = 30): # Function to get individual stats for one car
    ...
    # Account for potential string placeholders for winning car, but this should not be needed for the distance race
    yList = []
    xList = []
    areaList = []
...
distanceRaceStats(raceHistory) # Call the function to gather the full race stats
